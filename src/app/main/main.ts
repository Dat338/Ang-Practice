...
daaklika(categoryId: number) {
  console.log(categoryId)
  this.filteredItems = this.items.filter(item => item.categoryId === categoryId);
}
spiceLevel = 0
filterSpice(spice : number) {
  this.filteredItems = this.items.filter(item => item.spiciness === spice)
}
...
rangeprice () {
  ...
  let minCheck = this.min === null || item.price >= this.min;
  let maxCheck = this.max === null || item.price <= this.max;
